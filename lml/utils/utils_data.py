...
def data_loader_binarytask_rep(path_to_data, num_classes, info_file_name, num_repetition, num_unlabel_data, num_validation_data=-1, flatten_img=True, use_true_label=False, confidence_threshold=0.0, baseline_config=0, noise_level=0.0, dpsscl_baseline='none'):
    if dpsscl_baseline.lower() == 'mv':
        print("\tDPSSCL baseline - majority vote")
    elif dpsscl_baseline.lower() == 'snorkel':
        print("\tDPSSCL baseline - snorkel")
    elif dpsscl_baseline.lower() == 'repeated':
        print("\tDPSSCL baseline - repeated labeling")
...
def data_loader_binarytask(path_to_data, num_classes, info_file_name, num_unlabel_data, num_validation_data=-1, flatten_img=True, use_true_label=False, confidence_threshold=0.0, baseline_config=0, noise_level=0.0, dpsscl_baseline='none'):
    ...
    if baseline_config == 1:  # Special case for class selection with ORDISCO baseline experiments
        ...
        for class_cnt0 in range(num_classes-1):
            for class_cnt1 in range(class_cnt0+1, num_classes):
                ...
                train_x, valid_x = np.transpose(np.concatenate((x_l, x_u[indices_train_unlabeled]), axis=0), (0, 2, 3, 1)), np.transpose(x_u[indices_valid_unlabeled], (0, 2, 3, 1))
                if use_true_label:
                    # Use true labels instead of DPSSCL-generated labels
                    train_y, valid_y = np.concatenate((y_l, y_u[indices_train_unlabeled]), axis=0), y_u[indices_valid_unlabeled]
                else:
                    # Use DPSSCL-generated labels
                    if noise_level > 0.0:
...
def data_loader_multiclass_instance_incremental(path_to_data, num_classes, info_file_name, num_unlabel_data, num_validation_data=-1, flatten_img=True, use_true_label=False, dataset='mnist'):
    ...
    if 'mnist_dpsscl' in dataset:
        dataset_name = '5-9'
        incr_batch_size = 1000
    elif 'cifar10_dpsscl' in dataset:
        dataset_name = '0-9'
        incr_batch_size = 1000
    ...
    # ***** Filter data to remove low-confidence DPSSCL-labeled instances *****
