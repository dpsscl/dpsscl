...
from lml.classification.train import train_lifelong, train_den_net
from lml.classification.train_with_transferred_wlf import train_with_transferred_wlf
...
def gen_dir_to_save_weak_labelers(path_to_wls_root, result_name, exp_type):
    if 'cifar10' in exp_type:
        assert (result_name not in listdir(path_to_wls_root)), "Weak labelers exist!"
        new_result_path = join(path_to_wls_root, result_name)
        mkdir(new_result_path)
        for task_cnt1 in range(9):
            for task_cnt2 in range(task_cnt1+1, 10):
                new_dir_name = '%s_%s'%(task_cnt1, task_cnt2)
                mkdir(join(new_result_path, new_dir_name))
    else:
...
def train_run_for_each_model(model_architecture, model_hyperpara, train_hyperpara, dataset, data_type, mat_file_name, classification_prob, saved_result=None, useGPU=False, GPU_device=0, doLifelong=False, doSemiLifelong=False, saveParam=False, saveParamDir=None, saveGraph=False, tfInitParamPath=None, task_order=None, debug_data_group=-1):
    ...
    for run_cnt in range(run_cnt_init, max_run_cnt):
        ...
        if ('den' in model_architecture) or ('dynamically' in model_architecture):
            ...
            if not 'DPSSCL_wls' in listdir(RESULT_ROOT):
                mkdir(join(RESULT_ROOT, 'DPSSCL_wls'))
            mat_file_name_wo_ext = mat_file_name[:mat_file_name.rfind('.')]
            gen_dir_to_save_weak_labelers(join(RESULT_ROOT, 'DPSSCL_wls'), "%s_r%d" % (mat_file_name_wo_ext, run_cnt), data_type)
            WLS_ROOT = join(RESULT_ROOT, 'DPSSCL_wls', "%s_r%d" % (mat_file_name_wo_ext, run_cnt))
            train_result_tmp, num_model_para = train_with_transferred_wlf(model_architecture, model_hyperpara, train_hyperpara, [train_data, valid_data, test_data], data_type, classification_prob, useGPU, GPU_device, save_param=saveParam, param_folder_path=save_param_path, save_graph=(saveGraph and run_cnt < 1), run_cnt=run_cnt, task_order=task_order_this_run, weak_labeler_root=WLS_ROOT)
        else:
