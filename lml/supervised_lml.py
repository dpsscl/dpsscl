...
def supervised_lml_main():
    ...
    parser.add_argument('--use_true_label', help='Use true label of unlabeled data for training (instead of label from DPSSCL labelers', action='store_true', default=False)
    ...
    parser.add_argument('--set_dpsscl_conf_threshold', help='Set user-specified confidence threshold on DPSSCL labels', action='store_true', default=False)
    parser.add_argument('--dpsscl_conf_threshold', help='User-specified confidence threshold on DPSSCL labels', type=float, default=0.51)
    parser.add_argument('--dpsscl_noise', help='Noise on DPSSCL labels', type=float, default=0.0)
    parser.add_argument('--dpsscl_baseline', help='Baselines to compare against DPSSCL labels (mv/repeated/snorkel)', type=str, default='none')

    parser.add_argument('--transfer_wls', help='Transfer the previously learned weak labelers after evaluating suitability', action='store_true', default=False)
    parser.add_argument('--finetune_wls', help='Fine-tune the transferred weak labelers', action='store_true', default=False)
    parser.add_argument('--dpsscl_transfer_score_threshold', help='Threshold of transferability score to exclude less correlated tasks', type=float, default=1.0)
    parser.add_argument('--dpsscl_transfer_knowledge_prob', help='Chance to use earlier tasks weak labelers instead of training them from scratch', type=float, default=0.0)
    parser.add_argument('--weak_labelers_min', help='Minimum number of weak labelers to generate', type=int, default=5)
    parser.add_argument('--weak_labelers_max', help='Maximum number of weak labelers to generate', type=int, default=25)
    parser.add_argument('--weak_labelers_keep', help='Number of weak labelers to keep after each generation process', type=int, default=10)
    parser.add_argument('--unified_score', help='Use the score combining transferability score and F1-score for weak labeler transfer', action='store_true', default=False)

    parser.add_argument('--debug', help='Run whole code in debugging mode - evaluating various metrics during training', action='store_true', default=False)
    ...
    train_hyperpara['dpsscl_transfer_score'] = args.transfer_score_type
    ...
    # additional hyperparameters for DPSSCL-CL integrated pipeline
    train_hyperpara['task_similarity_score_threshold'] = args.dpsscl_transfer_score_threshold
    train_hyperpara['sample_lf_from_earlier_task_prob'] = args.dpsscl_transfer_knowledge_prob
    train_hyperpara['strong_labeler'] = args.dpsscl_baseline
    train_hyperpara['num_unlabeled_data'] = data_num_unlabel
    train_hyperpara['num_weak_labelers_min'] = args.weak_labelers_min
    train_hyperpara['num_weak_labelers_max'] = args.weak_labelers_max
    train_hyperpara['num_weak_labelers_keep'] = args.weak_labelers_keep
    train_hyperpara['unified_score'] = args.unified_score
    train_hyperpara['finetune_wls'] = args.finetune_wls
    train_hyperpara['debug_mode'] = args.debug

    if 'mnist_dpsscl' in data_type:
        ...
        if args.instance_incremental:
            # Instance-incremental experiment set-up
            info_file_name = 'dpsscl_mnist_ii'
            train_data, validation_data, test_data = data_loader_multiclass_instance_incremental_rep(getcwd() + '/Data/dpsscl_labels/mnist_inst_incr_labels', args.num_classes, info_file_name, train_hyperpara['num_train_valid_data_group'], data_num_unlabel, use_true_label=use_true_label, dataset=data_type)
            ...
            # Supervised lifelong learning experiment set-up
            ...
            if args.set_dpsscl_conf_threshold:
                label_conf_threshold = min(max(args.dpsscl_conf_threshold, 0.0), 1.0)
            else:
                label_conf_threshold = 0.51
            info_file_name = 'dpsscl_mnist_u'+ (str(data_num_unlabel) if data_num_unlabel > 0 else str(0))
            train_data, validation_data, test_data = data_loader_binarytask_rep(getcwd() + '/Data/dpsscl_labels/mnist_labels', 10, info_file_name, train_hyperpara['num_train_valid_data_group'], data_num_unlabel, use_true_label=use_true_label, confidence_threshold=label_conf_threshold, noise_level=args.dpsscl_noise, dpsscl_baseline=args.dpsscl_baseline, flatten_img=False)

        mnist_data_print_info(train_data, validation_data, test_data)
        classification_prob=True

    elif 'cifar10_dpsscl' in data_type:
        ...
        if args.instance_incremental:
            # Instance-incremental experiment set-up
            info_file_name = 'dpsscl_cifar10_ii'
            train_data, validation_data, test_data = data_loader_multiclass_instance_incremental_rep(getcwd() + '/Data/dpsscl_labels/cifar10_inst_incr_labels', args.num_classes, info_file_name, train_hyperpara['num_train_valid_data_group'], data_num_unlabel, use_true_label=use_true_label, dataset=data_type)
            data_hyperpara['num_tasks'] = len(train_data[0])
            train_task_order = list(range(data_hyperpara['num_tasks']))
            if do_lifelong:
                train_hyperpara['patience'] = 5
            ...
            if args.ordisco_ci_baseline:
                # Class-incremental experiment set-up
            ...
            if args.set_dpsscl_conf_threshold:
                ...
                label_conf_threshold = 0.51
            info_file_name = 'dpsscl_cifar10_u' + (str(data_num_unlabel) if data_num_unlabel > 0 else str(0))
            train_data, validation_data, test_data = data_loader_binarytask_rep(getcwd() + '/Data/dpsscl_labels/cifar10_labels', 10, info_file_name, train_hyperpara['num_train_valid_data_group'], data_num_unlabel, use_true_label=use_true_label, confidence_threshold=label_conf_threshold, baseline_config=baseline_config, noise_level=args.dpsscl_noise, dpsscl_baseline=args.dpsscl_baseline, flatten_img=False)
        cifar_data_print_info(train_data, validation_data, test_data)
        classification_prob=True

    elif 'cifar100_dpsscl' in data_type:
        data_hyperpara['image_dimension'] = [32, 32, 3]
    ...
    # Training the Model
    saved_result = train_run_for_each_model(model_architecture, model_hyperpara, train_hyperpara, [train_data, validation_data, test_data], data_type, mat_file_name, classification_prob, saved_result=None, useGPU=use_gpu, GPU_device=gpu_device_num, doLifelong=do_lifelong and not args.transfer_wls, doSemiLifelong=args.transfer_wls, saveParam=saveparam, saveParamDir=save_param_path, saveGraph=args.savegraph, tfInitParamPath=None, task_order=train_task_order, debug_data_group=args.data_group)
