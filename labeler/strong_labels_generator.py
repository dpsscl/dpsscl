...
# Obtain label matrix
def get_label_matrix(task, task_dir, lfs, n_u=None, data=None):
...
def generate_strong_label(task, task_dir, cardinality, n_u=None, method='mv', data=None):
    lfs = get_labelers(task, task_dir)
    if method == 'mv':
        L_u, L_l, y_l = get_label_matrix(task, task_dir, lfs, n_u=n_u, data=data)
...
def generate_strong_labels(task, method='mv'):
    ...
    for d in os.listdir(task_parent_dir):
        ...
        for n_u in n_u_space:
            y_u_prime = generate_strong_label(task, task_dir, cardinality, n_u, method=method)
            if n_u is None:
