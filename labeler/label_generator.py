...
## function and package to compute various scores for debugging
from sklearn.metrics import f1_score
def _calculate_jaccard_distance(num_labeled_total, num_labeled_L):
...
def _calculate_coverage(marginals, b, ground):
...
class LabelGenerator:
                    ...
                    for lr in lr_search_space:
                        ...
                        if self.task in ['mnist', 'fashion', 'cifar10']:
                            model = LeNetWeakLabeler(in_dim_c=X.shape[1], in_dim_h=X.shape[2],
                                                     ...
                                                     dict_training_param=dict_training_params)
                        else:
                            raise NotImplementedError
    ...
    def generate_snuba_lfs(self, log_file, sample_earlier_lfs=False, sample_earlier_lfs_prob=0.5, path_earlier_lfs=None, task_similarity_score=None, task_similarity_score_threshold=0.0, return_snuba_iter_cnt=False, unified_score=False, finetune_transferred_wls=False, debug_mode=False):
        ...
        with open(log_file, 'a') as f:
            ...
            while len(hg.hf) < self.max_labelers:
                ...
                # get new lf by either training from scratch or fine-tuning lf of closely related earlier tasks
                if sample_earlier_lfs:
                    hg.run_synthesizer(max_cardinality=1, idx=idx, keep=self.keep, model='cnn', sample_earlier_lfs=sample_earlier_lfs, sample_earlier_lfs_prob=sample_earlier_lfs_prob, task_similarity_score=task_similarity_score, task_similarity_score_threshold=task_similarity_score_threshold, path_earlier_lfs=path_earlier_lfs, unified_score=unified_score, finetune_transferred_wls=finetune_transferred_wls)
                    hg.run_verifier()
                    num_lf_prev_iter, num_lf_now = len(old_lf), len(hg.hf)
                    if num_lf_now > num_lf_prev_iter:
                        num_new_lfs = num_lf_now - num_lf_prev_iter
                        new_lf = hg.hf[-num_new_lfs:]
                        old_lf = hg.hf[0:num_lf_prev_iter]
                    else:
                        continue
                        #raise ValueError("Failed to get new weak labelers!")
        ...
        if return_snuba_iter_cnt:
            return hg.hf, cnt_snuba_iter
        else:
            return hg.hf
